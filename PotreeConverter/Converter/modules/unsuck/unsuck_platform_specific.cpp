...
MemoryData getMemoryData() {
	...
	data.physical_used = 0; // Not easily available on macOS
	data.physical_usedByProcess = physMemUsedByMe;
	data.physical_usedByProcess_max = physicalUsedMax;

	data.virtual_total = 0; // Not easily available on macOS
	data.virtual_used = 0;
	data.virtual_usedByProcess = virtualMemUsedByMe;
	data.virtual_usedByProcess_max = virtualUsedMax;
...
CpuData getCpuData() {
	...
	CpuData data;
	data.numProcessors = numProcessors;
	data.usage = 0.0; // CPU usage tracking is complex on macOS, simplified for now
